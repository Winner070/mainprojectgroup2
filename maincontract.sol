...
contract tech4dev is IERC20{
   ...
   uint public override totalSupply = 10**6 * 10**18; //this is the total supply the token
   mapping(address => uint) public override balanceOf; //get the balance of owner
   mapping(address => mapping(address => uint)) public override allowance; //to get the address of the spender and the amount
   string public name = "tech4dev Token"; //token name that will appear in the contract
   string public symbol = "T4D";        //symbol of the token
   uint public decimals = 18;         //decimal of the token

   function transfer(address recipient, uint amount) external override returns(bool){ //indicate the amount deducted from the address of the person who owns the contract
   balanceOf[msg.sender] -= amount; //deduct the amount from sender

   balanceOf[recipient] += amount;//indicate the deducted token from the sende's address 
